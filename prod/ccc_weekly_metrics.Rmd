...
print("Authentification to BQ Complete!")
...
nvar = floor((length(recrvar_d$column_name))/6) ##to define the number of variables in each sql extract from GCP
nvar
...
# Start column for each split data frame
   ...
   for (i in (1:length(start)))  {
     ...
     print(paste0("Downloading chunk ", i, " of ", length(start), " from BQ participants table."))
     recrbq[[i]] <- bq_table_download(tmp, bigint="integer64",max_results = Inf, page_size = 12)
   }
   print("BQ participants table download complete!!")
...
#  to check variables in recr_noinact_wl1
 ...
 # recr_noinact_wl1 <- readRDS("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit2023-05-22.rds" )
...
#Table 51 (daily Table 18.) Sex of All Records
...
# Notes: Hi Jing, the KPs requested that we add to this bar chart an additional category to track 'completes' against the TO6 goals. All the other categories will stay the same but you will need to add a 4th category for completes. Please use the same variables to calculate this that you used in the weekly log. Please also add a footnote that states: 'Completed=all 4 sections of the initial survey + samples donated (for research collections: urine, mouthwash, and any blood is collected; for clinical collections: urine and any blood collected).
 ...
 recruit.site1 <- recruit.site %>% 
      mutate(Site=Site,
             Expected = Expected,
             across(c("Verified","Blood","Initial Srv+Samples"), ~col_pct(.x, Expected)))
