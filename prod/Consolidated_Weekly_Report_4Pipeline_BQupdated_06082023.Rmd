...
knit: (function(inputFile, encoding) {rmarkdown::render("Consolidated_Weekly_Report_4Pipeline_BQupdated_06082023.Rmd", output_file=paste("Connect_WeeklyOperationsMetrics_", Sys.Date()-1,"_boxfolder141543281606", ".pdf", sep=""))})
...
ca_hist <- data %>% filter(.,d_699625233=='353358909' & d_452166062 == '353358909' ) %>% select(token,Connect_ID,d_821247024, d_512820379, site, age,sex, race, d_430551721,d_471593703,  d_699625233,d_452166062,all_of(ca.var.hist)) %>%mutate(`Verification status`= case_when(d_821247024 == '875007964' ~ "Not yet verified",                                              d_821247024 == '197316935'  ~ "Verified",                                              d_821247024 ==  '219863910' ~ "Cannot be verified",                                              d_821247024 ==  '922622075' ~ "Duplicate",                                              d_821247024 ==  '160161595' ~ "Outreach timed out"),            `Recruitment type` =case_when(d_512820379 == '486306141' ~ "Active",                                                 d_512820379 == '854703046' ~ "Passive"),            `User Profile submitted` = case_when(d_699625233 =='353358909' ~ "Yes",                                                 d_699625233 =='104430631' ~ "No"),            `Previous Cancer` = case_when(d_452166062 =='353358909' ~ "Yes",                                          d_452166062 =='104430631' ~ "No"),             RcrtV_CStatusMatch_v1r0 = case_when(state_d_547895941== '539025306' ~ "Criterium not met",                                                state_d_547895941== '427405444' ~ "Criterium met"),            RcrtUP_SubmitTime_v1r0 = ymd_hms(d_430551721),            RcrtSI_TypeTime_v1r0 = ymd_hms(d_471593703)) %>%   dplyr::rename(`Cancer Year` = d_650597106, `Cancer Type`=d_266952173, `Cancer Comments`= d_494982282)      
...
# Assuming refwd.cases.notes$site is a factor variable
site_levels <- levels(refwd.cases.notes$site)

# Create a data frame to pair dir_ids and site_strings
dir_site_pairs <- data.frame(
  dir_id = c(141622949146, 141623034772, 141623623841, 141622125003, 141623855895, 
             142482879711, 142480938244, 142481904303, 142481813411),
  site_string = c("HealthPartners", "Henry Ford Health System", 
                  "Marshfield Clinic Health System", "Sanford Health", 
                  "University of Chicago Medicine", "Kaiser Permanente Colorado", 
                  "Kaiser Permanente Georgia", "Kaiser Permanente Hawaii", 
                  "Kaiser Permanente Northwest")
)

# Choose whether to write the CSV files to Box or locally
write_to_box <- FALSE

for (i in seq_along(site_levels)) {
  site <- site_levels[i]
  
  # Extract data subset for the current site
  data_subset <- refwd.cases.notes[which(refwd.cases.notes$site == site), 
                                  which(!grepl("Date", colnames(refwd.cases.notes)))]
  
  # Generate the file name for the current site's data subset
  file_name <- paste("RefusalWithdrawl_cases_", site, "_", currentDate, "_boxfolder", dir_site_pairs$dir_id[i], ".csv", sep="")
  
  if (write_to_box) {
    # Box folder and write for each site
    box_setwd(dir_id = dir_site_pairs$dir_id[i])
    box_write(object = data_subset, file_name = file_name, row.names = FALSE, na="")
  } else {
    # Set the output file path for the CSV file (local write)
    output_file <- paste(outputpath, file_name, sep="")
    # Write the data subset to a CSV file locally
    write.csv(data_subset, output_file, row.names = FALSE, na="")
  }
}
...
write.csv(incentive[,c("token","site","NORC.Payment.Eligibility","Eligible.for.Payment","Time.Payment.Eligible.for.Round")],paste(outputpath,"Incentives_Weekly_Report_NORC",currentDate,"_boxfolder167284102961",".csv", sep=""),na="",row.names=F)
...
write.csv(incentive_nonchi[,c("token","site","NORC.Payment.Eligibility","Eligible.for.Payment","Time.Payment.Eligible.for.Round")],paste(outputpath,"Incentives_Weekly_Report_OTHERSITES_",currentDate,"_boxfolder167284102961",".csv", sep=""),na="",row.names=F)
...
write.csv(base,paste(outputpath,"CCC_metrics_weekly_log_labels_updated_",currentDate,"_boxFolder",box_dir_id,".csv",sep=""),na="",row.names=F)
...
for(i in 1:length(site)){
  ...
  write.csv(sub.base[which(subs.recr_veri_wk$Week.Starting != currentDate),],paste(outputpath,"CCC_metrics_weekly_log_labels_",site[i],currentDate,"_boxfolder",box_dir,".csv", sep=""),na="",row.names=F)
  ...
  write.csv(subs.recr_veri_wk,paste(outputpath,"CCC_metrics_weekly_log",site[i],currentDate,"_boxfolder",box_dir,".csv", sep=""),na="",row.names=F)
